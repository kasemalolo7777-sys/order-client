...
function DateInput({
  ...
  const formatDate = (date: Date) => {
   ...
   const formatInputValue = (val: string | null) => {
    if (!val) return '';
    
    if (inputType === 'week') return val;
...
useEffect(() => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (inputType === 'week') {
      setDate(rawValue);
      handleChangeEvent(rawValue, name);
      return;
    }
    ...
    if (rawValue) {
      ...
      if (!isNaN(dateObj.getTime())) {
        formattedValue = formatDate(dateObj);
      }
    }

    setDate(formattedValue);
    handleChangeEvent(formattedValue, name);
  };
   const {isAppleDevice,mobileOS} = useViewport()
  return (
    <div
      className={cn(
    ...
    >
      {title && (
        <p className="flex text-sm  text-primary dark:text-white dark:bg-primary">
          {ar?<>
            {isRequired && <pre className="text-red-500">*</pre>} {title}
      ...
      <input
        ...
        min={min || ""}
        max={max || ""}
        className={`w-0 h-0 absolute top-0 ${isAppleDevice && 'opacity-0'}`}
        name={name}
        id={name}
        value={formatInputValue(value || dateInputProps.value)
}
